        ...
        if (xhr.readyState==4 && xhr.status==200) {
            ...
            for (let i = 0; i < fireData.response.body.totalCount; i++) {
                ...
                <h2 class="accordion-header" id="heading${accordionIndex[i]}">
                  <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse${accordionIndex[i]}" aria-expanded="true" aria-controls="collapse${accordionIndex[i]}">
                    전국 산불 위험 예보 ${fireData.response.body.items.item[i].analdate}
                  </button>
                </h2>
                <div id="collapse${accordionIndex[i]}" class="accordion-collapse collapse show" aria-labelledby="heading${accordionIndex[i]}" data-bs-parent="#accordionExample">
